...
4. Open the **index.html** file & replace **"Pokémon"** with the artist/genre/game of your heardle.
...
8. On line **1** of **translation.js** file and update **languages** with the list of languages of your Heardle.
9. On line **3** of **music-list.js**, "var **musicNameList**" is initialized. Replace it with a list of music titles you'd like to appear as suggestions in your Heardle. The list should follow the format: **`{ id : music id, codeLanguage : "Music title" }`**. Each music must have a different id, and there must be one entry per language. For each language, music titles should follow the format **"Music - Artist/Genre/Game"**. Make sure each music is in quotes and has a comma after it.
